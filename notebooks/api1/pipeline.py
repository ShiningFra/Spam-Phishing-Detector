...
# ── Correcteur incremental ────────────────────────────────────────
def _load_corrector():
    """Charge le correcteur incremental si disponible, sinon None."""
    if CORRECTOR_PATH.exists():
        try:
            corrector = joblib.load(CORRECTOR_PATH)
            logger.info("Correcteur incremental charge.")
            return corrector
        except Exception as e:
            logger.warning(f"Correcteur non charge : {e}")
    return None

def _apply_corrector(corrector, rf_proba_dict: dict,
                     X_tfidf, class_names: list) -> dict:
    """
    Fusionne les probabilites RF avec le correcteur incremental.
    RF conserve 60-100% du poids selon la confiance du correcteur.
    Retourne un dict {classe: proba} normalise.
    """
    rf_proba = np.array([rf_proba_dict.get(c, 0.0) for c in class_names])

    try:
        # Features du correcteur : proba RF + TF-IDF reduit
        ...
        X_corr = hstack([csr_matrix([rf_proba]), X_small])

        if hasattr(corrector, 'predict_proba'):
            corr_proba = corrector.predict_proba(X_corr)[0]
        else:
            return rf_proba_dict  # Pas de proba disponible, RF seul

        # Fusion dynamique : poids correcteur proportionnel a sa confiance
        corr_conf = float(corr_proba.max())
        w_corr    = 0.4 * corr_conf   # 0 a 40% selon confiance
        w_rf      = 1.0 - w_corr
        ...
        logger.warning(f"Correcteur erreur, RF seul : {e}")
...
class HybridEmailPipeline:
    ...
    def __init__(self):
        ...
        self._corrector     = None   # Couche incrementale
    ...
    def load_models(self):
        ...
        self._corrector     = _load_corrector()  # None si pas encore cree

        logger.info(f"Modele base : {type(self._ml_model).__name__}")
        logger.info(f"Classes     : {self._class_names}")
        logger.info(f"Correcteur  : {'actif' if self._corrector else 'inactif (aucune correction encore)'}")

    def reload_corrector(self):
        """Recharge le correcteur sans redemarrer l'API. Appele apres update."""
        self._corrector = _load_corrector()
        logger.info(f"Correcteur recharge : {'actif' if self._corrector else 'inactif'}")

    def _predict_ml(self, text: str):
        """Retourne (proba_dict, X_tfidf) pour reutilisation par le correcteur."""
    ...
    def analyze(self, text: str, raw_email: str = '') -> AnalysisResult:
        ...
        # Couche 3 : ML (Random Forest)
        try:
            ml_proba, X_tfidf = self._predict_ml(text)
        except Exception as e:
            logger.warning(f"ML predict failed: {e}")
            ml_proba = {c: 1.0/len(self._class_names) for c in self._class_names}
            X_tfidf  = None

        # Couche 3b : Correcteur incremental (si disponible)
        if self._corrector is not None and X_tfidf is not None:
            ml_proba = _apply_corrector(
                self._corrector, ml_proba, X_tfidf, self._class_names
            )
            path.append(f'corrector({max(ml_proba,key=ml_proba.get)})')
        else:
...
class PipelineLoader:
    ...
    def reload_corrector(self):
        """Recharge le correcteur sans redemarrer — appeler apres incremental_layer.py."""
    ...
    def get_models_info(self) -> dict:
        return {
            'pipeline':    'HybridEmailPipeline (Random Forest + correcteur incremental)',
            'ml_model':    type(self._pipeline._ml_model).__name__ if self._pipeline._ml_model else 'non charge',
            'corrector':   'actif' if self._pipeline._corrector else 'inactif',
            'bert':        False,
