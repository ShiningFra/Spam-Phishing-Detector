...
def update_corrector_v2(corrections, rf_model, tfidf, force=False):
    ...
    # ── Migration automatique des dimensions ──────────────────────────
    # Si le correcteur existant a été entraîné avec 508 features (v1)
    # et qu'on tente un partial_fit avec 523 features (v2), sklearn lève une erreur.
    # On détecte l'incompatibilité et on réinitialise le correcteur.
    # Le réentraînement sur les corrections actuelles restaure les connaissances.
    from sklearn.linear_model import SGDClassifier as _SGD
    expected_features    = X.shape[1]
    corrector_features   = getattr(corrector, 'n_features_in_', None)
    if corrector_features is not None and corrector_features != expected_features:
        logger.warning(
            f"Migration correcteur : {corrector_features} → {expected_features} features. "
            "Réinitialisation automatique et réentraînement sur corrections disponibles."
        )
        ts     = datetime.now().strftime("%Y%m%d_%H%M%S")
        backup = BACKUP_DIR / f"corrector_PRE_MIGRATION_{ts}.pkl"
        if CORRECTOR_PKL.exists():
            shutil.copy2(CORRECTOR_PKL, backup)
            logger.info(f"Backup avant migration : {backup.name}")
        corrector = _SGD(
            loss="modified_huber", alpha=0.001,
            max_iter=100, tol=1e-4, random_state=42,
        )
        logger.info(f"Nouveau correcteur SGD ({expected_features} features).")
