...
SPAM_LEXICON = {
    ...
    'marketing': [
        ...
        # Variantes francaises (urgence / appel a l'action)
        'cliquez ici','cliquez vite','cliquer ici','offre limitee',
        'offre exceptionnelle','derniere chance','ne manquez pas',
        'felicitations vous avez','vous avez ete selectionne',
        'gratuit aujourd hui','profitez en vite','depechez vous',
    ],
    ...
    'scam': [
        ...
        'heritage','fonds bloques','transfert urgent','colis bloque',
        'compte bloque','reglement immediat','penalite de retard',
        'mise a jour obligatoire','dernier avertissement',
    ],
}
...
class HeuristicLayer:
    ...
    def analyze(self, text: str, is_whitelisted: bool = False
        ...
        # ── Phishing : URLs ──────────────────────────────────────
        # Capture les URLs avec OU sans schema (www.exemple.com seul, courant
        # dans les emails d'arnaque qui omettent http:// pour paraitre informels)
        urls = re.findall(r'(?:https?://)?(?:www\.)?[a-zA-Z0-9-]+\.[a-zA-Z]{2,}(?:/\S*)?', text)
        # Filtrer les faux positifs (emails, ex: user.name@domain.com deja gere ailleurs)
        urls = [u for u in urls if '@' not in u or u.count('@') == 0]

        SCAM_DOMAIN_PATTERNS = [
            'scam', 'arnaque', 'phishing', 'fake', 'verify-account',
            'secure-update', 'account-confirm', 'login-verify',
        ]

        for url in urls:
            try:
                # Reconstruire un netloc meme sans schema
                if not url.startswith(('http://', 'https://')):
                    candidate = 'http://' + url
                else:
                    candidate = url
                parsed = urlparse(candidate)
                h = parsed.netloc.lower().lstrip('www.')
                ...
                # Domaine contenant explicitement un mot d'arnaque
                # (ex: scamarnaques.com, fake-paypal.xyz) -> tres fort signal
                for pat in SCAM_DOMAIN_PATTERNS:
                    if pat in h:
                        phish_pts += 0.55
                        url_flags.append(f'scam_domain_keyword:{pat}@{h}')
                        break
...
def _get_corrector_n_features(corrector) -> int:
    """Détecte le nombre de features attendu par le correcteur existant."""
    if hasattr(corrector, 'n_features_in_'):
        return int(corrector.n_features_in_)
    if hasattr(corrector, 'coef_'):
        return int(corrector.coef_.shape[1])
    return 523   # Défaut v2


def _build_corrector_input(rf_proba, X_small, struct_feats: dict,
                            n_expected: int):
    """
    Construit le vecteur d'entrée du correcteur en s'adaptant
    automatiquement aux dimensions attendues.

    Dimensions possibles :
      508 = 3 (RF) + 500 (TF-IDF) + 5 (struct légères v1)
      523 = 3 (RF) + 500 (TF-IDF) + 20 (struct complètes v2)
    """
    struct_values = list(struct_feats.values())   # 20 valeurs

    if n_expected == 523:
        # v2 : struct complètes (20)
        X_struct = csr_matrix([struct_values])
        return hstack([csr_matrix([rf_proba]), X_small, X_struct])

    elif n_expected == 508:
        # v1 : 5 struct légères — utiliser les 5 premières (url_count...has_at)
        X_struct_v1 = csr_matrix([struct_values[:5]])
        return hstack([csr_matrix([rf_proba]), X_small, X_struct_v1])

    elif n_expected == 503:
        # v1 ancienne : sans struct du tout
        return hstack([csr_matrix([rf_proba]), X_small])

    else:
        # Fallback : construire à la bonne taille par padding/troncature
        base = hstack([csr_matrix([rf_proba]), X_small])   # 503
        current = base.shape[1]
        if current < n_expected:
            pad = csr_matrix(np.zeros((1, n_expected - current)))
            return hstack([base, pad])
        else:
            return base[:, :n_expected]
...
def _apply_corrector(corrector, rf_proba_dict: dict,
    ...
    """
    Fusion RF + correcteur SGD avec adaptation automatique des dimensions.

    Résout l'erreur : "X has 523 features, but SGDClassifier is expecting 508"
    en détectant les dimensions du correcteur existant et en adaptant
    le vecteur d'entrée en conséquence.

    Lors de la prochaine mise à jour via incremental_layer_v2.py,
    le correcteur sera réentraîné avec 523 features et l'adaptation
    ne sera plus nécessaire.
    """
    rf_proba = np.array([rf_proba_dict.get(c, 0.0) for c in class_names])

    try:
        # TF-IDF réduit top-500
        ...
        # Détecter les dimensions du correcteur existant
        n_expected = _get_corrector_n_features(corrector)

        # Construire le vecteur d'entrée adapté
        X_corr = _build_corrector_input(rf_proba, X_small, struct_feats, n_expected)
...
class HybridEmailPipelineV2:
    ...
    def _aggregate(self, rule_spam: float, rule_phish: float,
        ...
        # ── Seuils calibrés — appliqués sur ml_proba BRUT (RF+SGD) ──────
        # Les seuils tau_phish=0.90 / tau_spam=0.60 ont ete calibres sur
        # les sorties brutes de Random Forest (cf. evaluation X_test).
        # Les appliquer sur `final` (apres fusion ponderee avec regles/
        # headers, qui ecrase les probas vers ~0.55) les rend
        # structurellement inatteignables. On les applique donc sur
        # ml_proba directement, puis on combine avec un score composite
        # qui integre les signaux regles/headers comme score additionnel
        # plutot que comme moyenne ponderee.
        # Recalibre empiriquement : ml_proba.phishing observe en pratique
        # dans la plage ~0.53-0.57 quasi quel que soit le contenu (le
        # correcteur SGD ecrase la dynamique de RF vers un plateau).
        # THRESH_PHISHING=0.65 est atteignable uniquement quand un signal
        # heuristique fort (rule_phish) s'ajoute au plateau ml (~0.55).
        # comp_spam = rule_spam directement, car ml_proba.spam est
        # negligeable (~0.0002) dans tous les cas observes.
        ...
        composite_spam  = min(rule_spam + head_vec.get('spam', 0), 1.0)

        p_phish = composite_phish
        p_spam  = composite_spam
        p_ham   = final.get('ham', ml_ham)
        ...
        if p_phish >= THRESH_PHISHING:
            ...
            # Fallback : decision par argmax sur le score `final` fusionne
            # (capture les cas ou aucun seuil n'est atteint mais ou le
            # signal fusionne penche clairement vers une classe non-ham)
    ...
    def analyze(self, text: str, raw_email: str = '') -> AnalysisResult:
        ...
        # Early stop combine : domaine scam explicite + signal coherent
        # (un domaine contenant litteralement "scam"/"arnaque"/"phishing" est
        # un signal quasi-deterministe ; pas besoin d'atteindre 0.95 si le
        # rule_phish depasse deja un seuil modere apres ce boost)
        scam_domain_hit = any('scam_domain_keyword' in f for f in url_flags)
        if scam_domain_hit and rule_phish >= 0.50:
            result.predicted_class   = 'phishing'
            result.global_confidence = max(rule_phish, 0.85)
            result.threat_level      = 'high'
            result.latency_ms        = (time() - t0) * 1000
            result.decision_path     = ' -> '.join(path) + ' -> EARLY_STOP_SCAM_DOMAIN'
            result.explanation       = self._explainer.explain(
                'phishing', max(rule_phish, 0.85), rules_hit, [], url_flags)
            return result
